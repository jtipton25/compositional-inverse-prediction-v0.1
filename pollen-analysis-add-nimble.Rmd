...
if (file.exists(here::here("model-fit", "fit-bummer-pollen.RData"))) {
  ...
  parallelChains <- function (n_chains) {
    ...
    ## MCMC fit
    sink(paste("~/mvgp/nimble/progress/fit-bummer-dm-", n_chains, ".txt", sep=""),
         append=TRUE)
  ...
  ## create temporary progress file  
  file.create(paste("~/mvgp/nimble/progress/fit-bummer-dm-", 
                    1:n_chains, ".txt", sep=""))
...
for (i in 1:N) {
  for (j in 1:nrow(alpha_post)) {
  p_alpha_post[j, i, ] <- alpha_post[j, i, ] / sum(alpha_post[j, i, ])
  }
}
...
fitPlotData <- data.frame(species = factor(rep(colnames(y), each=N-N_obs),
                                                levels=colnames(y)), 
                          count=c(y_prop[1:(N-N_obs), ]), 
                          depth=rep(X[1:(N-N_obs)], times=d), 
                          p=c(p_alpha_post_mean[1:(N-N_obs), ]),
                          p_lower = c(p_alpha_post_lower_95[1:(N-N_obs), ]),
                          p_upper = c(p_alpha_post_upper_95[1:(N-N_obs), ]))
fitPlotData$species <- revalue(fitPlotData$species, 
                               c("Sum.Other.Deciduous"= "DECIDUOUS",
                                 "Sum.Other.Herbaceous"= "OTHER HERBS",
                                 "Sum.Prairie.Herbs"= "PRAIRIE HERBS", 
