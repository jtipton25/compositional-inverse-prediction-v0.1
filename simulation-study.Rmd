...
% 
\mathbf{R} & = \operatorname{LKJ}(\boldsymbol{\xi}) \\
%
...
## Fit the Full model
This model took 3.6 hours running 4 mcmc chains in parallel on a 2017 iMac with 4.2GHz processor.
...
if (file.exists(here("/model-fit/fitDM.RData"))) {
  ...
  parallelChains <- function (n_chains) {
    ...
    out <- mcmc(mcmcRcpp(y[train, ], X[train], y[test, ],
                         ...
                         file_name="model-fit/progress/DM-sim.txt"))
    return(out)
  }
...
## Fit GAM DM model
This model took 36 minutes running 4 mcmc chains in parallel on a 2017 iMac with 4.2GHz processor.
...
if (file.exists(here("/model-fit/fit-dm-basis.RData"))) {
  ...
  parallelChains <- function (n_chains) {
    ...
    out <- coda::mcmc(mcmcRcpp(y[train, ], X[train], y[test, ], 
                               ...
                               file_name="model-fit/progress/DM-basis-sim.txt"))
  }
